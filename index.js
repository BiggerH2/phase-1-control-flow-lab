function scuberGreetingForFeet(distance: number, _customer: object) {
  if (distance <= 400) {
    return 'This one is on me!';
  } else if (distance <= 2000) {
    return `That will be ${(distance - 400) * 0.20} bucks.`;
  } else if (distance <= 3000) {
    return `I will gladly take your ${(distance - 2000) * 0.30} bucks.`;
  } else {
    return 'No can do.';
  }
}
...
describe('scuberGreetingForFeet()', function() {
  ...
  it('charges 20 dollars for a distance between 400 and 2000 feet', function() {
    expect(scuberGreetingForFeet(1500)).to.equal('That will be 30 bucks.');
  });

  it('charges 30 dollars for a distance over 2000 feet', function() {
    expect(scuberGreetingForFeet(2001)).to.equal('I will gladly take your 900 bucks.');
  });

  it('does not allow rides over 3000 feet', function() {
    expect(scuberGreetingForFeet(3001)).to.equal('No can do.');
  });
});

function ternaryCheckCity(city: string) {
  return city === 'NYC' ? 'Ok, sounds good.' : 'No go.';
}

function switchOnCharmFromTip(tip: string) {
  switch (tip) {
    case 'generous':
...
describre('ternaryCheckCity()', function() {
